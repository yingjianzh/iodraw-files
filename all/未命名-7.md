...
sequenceDiagram
    ...
    Frontend->>API Server: 请求用户组件列表
    API Server->>Database: 查询用户组件和数据
    Database->>API Server: 返回组件列表和数据状态
    API Server->>Frontend: 返回组件列表和数据状态
    Frontend->>User: 显示组件和数据

    alt 用户添加组件
        User->>Frontend: 点击添加组件按钮
        Frontend->>API Server: 添加组件请求
        API Server->>Database: 绑定组件与用户
    ...
    alt 用户更新组件数据
        User->>Frontend: 更新组件数据
        Frontend->>API Server: 更新数据请求
        API Server->>Database: 更新组件数据状态
        Database->>API Server: 确认更新
        API Server->>Frontend: 更新组件数据状态
