...
sequenceDiagram
    ...
    alt 用户未登录
        ...
        User->>API Server: 用户登录，建立会话
    end

    API Server->>LLM Server: request【用户需求】
    LLM Server-->>API Server: stream response【功能介绍】
    API Server->>User: stream【功能介绍】

    LLM Server-->>API Server: stream response【源码和依赖】
    API Server->>API Server: 缓存完整源码和依赖
    API Server->>API Server: 根据规则生成component_id
    API Server->>Module Generator: request【提交数据】code、libs、component_id
    Module Generator->>API Server: 返回打包结果(remote_url)

    API Server->>User: update【组件预览入口】
    API Server->>User: complete【添加组件按钮】
