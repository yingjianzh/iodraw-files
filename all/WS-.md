...
graph TB
  ...
  subgraph "App Backend"
    ...
    subgraph "核心服务"
      ...
      LLMProxy["LLM Proxy<br/>WebSocket连接管理"]
    end
  ...
  subgraph "LLM Server"
    LLMAPI["WebSocket /ws/chat/{session_id}<br/>双向通信"]
  end

%% ===== 用户发送消息流程 =====
  UI -->|"1.发送消息"| ChatAPI
  ChatAPI -->|"2.注册SSE连接<br/>获取EventChannel"| EventMgr
  ChatAPI -->|"3.发送消息"| LLMProxy

  LLMProxy -->|"4.获取/建立WS连接<br/>发送消息"| LLMAPI
  LLMAPI -.->|"5.WS推送响应<br/>(content/task_plan/tool_calls)"| LLMProxy

  LLMProxy -->|"6.解析WS消息<br/>转换为标准Event"| EventMgr
...
%% ===== 工具调用流程 =====
  ...
  ToolAPI -->|"12.发送工具结果"| LLMProxy
  ToolAPI -->|"13.立即返回200"| UI

  LLMProxy -->|"14.WS发送<br/>{role: tool}"| LLMAPI
  LLMAPI -.->|"15.WS推送响应"| LLMProxy
  LLMProxy -->|"16.解析消息"| EventMgr
